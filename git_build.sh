...
function setRstate {
	...
	mv NR-Uplink-Interference/build/feature-release.xml NR-Uplink-Interference/build/feature-release.${rstate}.xml
...
function Arm104nexusDeploy {
	...
	ArtifactId=$module
	zipName=NR-Uplink-Interference
	
	
	echo "****"	
	echo "Deploying the zip /$zipName-24.1.zip as ${ArtifactId}${rstate}.zip to Nexus...."
...
if [ $rsp == 0 ]; then
  ...
  git remote set-url --push origin ssh://esjkadm100@gerrit.ericsson.se:29418/OSS/ENIQ-CR-Parent/OSS/com.ericsson.eniq/NR-Uplink-Interference
